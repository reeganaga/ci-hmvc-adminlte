    ...
    public function proses_isi_kpi() {
        ...
        if ($id_penilaian) {
            foreach ($post['nilai'] as $id => $value) {
                ...
                // $data['nilai_target'] = $value['target']; revisi : nilai target diisi saat configure indicator
                $data['nilai_realisasi'] = $value['realisasi'];

                //calculate score
                $kpi_detail = $this->kpi_detail_model->get($id);
                $bobot = $kpi_detail->bobot;
                $nilai_target = $kpi_detail->nilai_target;
                //$score = calculate_score($value['realisasi'], $value['target'], $bobot);
                $score = calculate_score($value['realisasi'], $nilai_target, $bobot);
                // var_dump($score);
